...
export default function AccountSettings() {
  ...
  const fixSubscriptionData = trpc.stripe.fixMySubscriptionData.useMutation();
  ...
  // Handle fixing subscription data
  const handleFixSubscriptionData = async () => {
    setIsProcessing(true);
    try {
      const result = await fixSubscriptionData.mutateAsync();

      if (result.fixed) {
        alert(`Success! Your subscription data has been fixed. Customer ID: ${result.stripe_customer_id}\n\nPlease refresh the page.`);
        router.reload();
      } else {
      ...
      alert('Failed to fix subscription data. Please contact support.');
    } finally {
      setIsProcessing(false);
    }
  };
      ...
      >
        ...
        {hasActiveSubscription && (
          <div className="mt-4 space-y-4">
            ...
            {/* Troubleshooting: Show fix button if stripe_customer_id is missing */}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-amber-900 dark:text-amber-100 mb-1">
                      Subscription Data Issue Detected
                    </p>
                    <p className="text-sm text-amber-700 dark:text-amber-300 mb-3">
                      Your subscription is missing some Stripe connection data. Click below to fix this automatically.
                    </p>
                    <button
                      onClick={handleFixSubscriptionData}
                      disabled={isProcessing}
                      className="px-3 py-1.5 bg-amber-600 text-white rounded-md hover:bg-amber-700 text-sm font-medium transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
                    >
                      {isProcessing ? 'Fixing...' : 'Fix Subscription Data'}
                    </button>
                  </div>
                </div>
              </div>
